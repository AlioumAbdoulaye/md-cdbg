...
% --- compact spacing to fit page limit (cosmetic only) ---
...
\author{
\IEEEauthorblockN{Alioum Abdoulaye}
\IEEEauthorblockA{\textit{DAVID Lab, Universit\'e Paris-Saclay, UVSQ}\\
\textit{Dept. of Math. and Computer Science,}\\
\textit{University of Ngaound\'er\'e, Cameroon}\\
aliyoumabdoulaye@gmail.com}
\and
\IEEEauthorblockN{Yacine Benallouche}
\IEEEauthorblockA{\textit{Orange Research}\\
\textit{}\\
Paris, France\\
yacine.benallouche@orange.com}
\and
\IEEEauthorblockN{Zainab Issa Ahmad}
\IEEEauthorblockA{\textit{DAVID Lab, Universit\'e Paris-Saclay, UVSQ}\\
\textit{Dept. of Math. and Computer Science,}\\
\textit{University of Ngaound\'er\'e, Cameroon}\\
znamadina@gmail.com}
...
\IEEEauthorblockA{\textit{DAVID Lab, Universit\'e Paris-Saclay, UVSQ}\\
\textit{Versailles, France}\\
mourad.gueroui@uvsq.fr}
\and
\IEEEauthorblockN{Ado Adamou Abba Ari}
\IEEEauthorblockA{\textit{DAVID Lab, Universit\'e Paris-Saclay, UVSQ}\\
\textit{LaRI Lab, University of Maroua, Cameroon}\\
...
\emph{Multi-Domain Cyber-Deception Bayesian Game} (MD-CDBG), a stochastic
...
solution concept, the \emph{Resilience-constrained Bayesian Nash Equilibrium}
...
baselines---including a recent state-of-the-art double-oracle deception
approach---across
...
have already compromised critical assets. \emph{Cyber-deception} offers a
...
three structural gaps when applied to IoT. \textbf{First}, they typically
model a \emph{single} domain or layer, ignoring the cross-layer coupling that
...
making decoys easy to unmask. \textbf{Second}, many adopt a \emph{zero-sum}
...
\textbf{Third}, and most critically, prior work largely ignores
\emph{resilience}: the capacity of the system to maintain service continuity
under attack, which is the paramount concern for critical IoT
deployments~\cite{he2026}.

\textbf{Contributions.} This paper addresses these gaps with the following
contributions:
\begin{itemize}
\item We formulate the \emph{Multi-Domain Cyber-Deception Bayesian Game}
...
\item We define the \emph{Resilience-constrained Bayesian Nash Equilibrium}
...
\item We design \textbf{R-BNE-DO}, a Bayesian Double Oracle algorithm whose
...
sensors as a \emph{non-zero-sum} game solved by double oracle with a greedy,
...
not zero-sum, they \emph{prove} it is strategically equivalent to a zero-sum
game (in the sense of Moulin and Vial~\cite{moulin1978}), which lets them solve each restricted
...
game is \emph{not} reducible to zero-sum, so the LCP becomes necessary rather
than optional.
...
architecture: the attacker progresses upward through the three layers
(constraint $C_1$); decoys at adjacent layers must remain coherent ($C_2$); and
the defender maintains a per-layer Bayesian belief $\mu_k^t(\theta_A)$ over the
attacker type, all under a global budget ($C_3$) and a resilience constraint
$\Omega(t)\geq\Omega_{\min}$.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig0.png}
\caption{Three-layer architecture of the MD-CDBG.}
\label{fig:arch}
\end{figure}
...
\begin{itemize}
\item \textbf{($C_1$) APT progression:} layer $k$ is accessible only if at
...
\item \textbf{($C_2$) Semantic coherence:} decoys at adjacent layers must be
...
\item \textbf{($C_3$) Global budget:} the total deployment cost across all
...
maintains a belief $\mu_k^t(\theta_A) \in \Delta(\Theta_A)$ \emph{per layer}
...
% ---------------------------------------------------------------
% Full proof (kept in comment for reviewers / extended version):
% Fix theta != theta_A^*. By Bayes' rule (Eq. 3), the likelihood
% ratio L_t(theta) = mu_k^t(theta)/mu_k^t(theta_A^*) updates as
%   L_{t+1}(theta) = L_t(theta) *
%       sigma_k^A(a_k^A(t)|theta) / sigma_k^A(a_k^A(t)|theta_A^*).
% Taking expectation under the TRUE type theta_A^* (the law that
% actually generates a_k^A(t)):
%   E_{theta_A^*}[ L_{t+1}(theta) | F_t ]
%     = L_t(theta) * sum_a sigma_k^A(a|theta_A^*) *
%         sigma_k^A(a|theta)/sigma_k^A(a|theta_A^*)
%     = L_t(theta) * sum_a sigma_k^A(a|theta) = L_t(theta).
% Hence {L_t(theta)} is a non-negative martingale under theta_A^*,
% so it converges a.s. to a finite limit (martingale convergence).
% Its expected log-increment is
%   E[ log( sigma_k^A(a|theta)/sigma_k^A(a|theta_A^*) ) ]
%     = -KL( sigma_k^A(.|theta_A^*) || sigma_k^A(.|theta) ) < 0,
% strictly negative because the distributions are distinct
% (identifiability) and KL>0. By the strong law of large numbers
% over the infinitely many visits to layer k, log L_t(theta) -> -inf,
% i.e. L_t(theta) -> 0. Summing over the finitely many theta != theta_A^*
% gives mu_k^t(theta_A^*) = 1/(1 + sum_theta L_t(theta)) -> 1. QED.
% ---------------------------------------------------------------
...
\label{eq:detection}
\end{equation}
where the \emph{active} component scales the attacker's probing investment by
its type-specific efficiency,
\begin{equation}
d_k^{\mathrm{eff}}(\sigma_k^A) = \sigma_k^A(a_k^{\mathrm{probe}})
\cdot \psi_k(\theta_A),
\end{equation}
and the \emph{passive} component reflects the intrinsic detectability of
...
detection only if it survives \emph{both} the active probe and the passive
...
strategies at equilibrium. Figure~\ref{fig:interaction} summarizes this per-turn
interaction as a five-step loop: \emph{(1)} the defender holds a belief
$\mu_k^t(\theta_A)$ while the attacker has a true type $\theta_A^*$;
\emph{(2)} both players act simultaneously on $f_1^k$ (the defender pushing it
up with better decoys, the attacker pushing it down through detection);
\emph{(3)} because $f_1^k$ both reduces the attacker's gain and raises its risk,
neither player can play a pure strategy, \emph{(4)} yielding a mixed Nash
equilibrium computed by the LCP; the resulting state transition updates the
resilience $\Omega(t)$; and \emph{(5)} the observed attacker action feeds the
Bayesian belief update for the next turn. The global attacker utility aggregates
over layers subject to the $C_1$ accessibility indicator:
\begin{equation}
...
\caption{Per-turn interaction between the two players (five-step loop).}
...
\emph{Resistance} indicates whether the uncompromised fraction exceeds $\rho_k$;
\emph{absorption} measures the remaining healthy fraction; and \emph{recovery}
...
three respects.
First, the defender strategy $\sigma^{D*}(\mu^t)$ is a \emph{belief-dependent
...
\emph{hard} resilience constraint at every time step. Third, condition~(ii) uses
the attacker's \emph{true} utility $U^A$---not the defender's
...
\end{align}
with step $\eta_t=\eta_0/\sqrt{t}$ satisfying the Robbins--Monro
conditions~\cite{robbins1951}
...
\emph{belief} $\mu_k^t$ (the defender does not know the type), whereas $M_k^A$
uses the attacker's \emph{true} type $\theta_A^*$. Second, the budget and
coherence penalties ($\lambda,\nu_k$) appear \emph{only} in $M_k^D$: they are
...
\emph{(i) Inner level:} for fixed multipliers, Bayes-DO terminates in finitely
many iterations at the exact restricted Nash equilibrium, computed by the
LCP~\cite{lemke1964}; assumption (A1) prevents cycling. \emph{(ii) Outer level:}
dual gradient ascent with diminishing steps converges to the saddle point of the
Lagrangian~\eqref{eq:lagrangian} under Slater's strong-duality condition~\cite{boyd2004}, which
...
$\nu_k^*>0$ only if $\Coh_k=\delta_{\min}$. \emph{(iii) Belief level:} by
...
\textbf{Python~3} using \texttt{NumPy} for the numerical core and \texttt{SciPy}
...
equilibria are computed by a dedicated \textbf{Linear Complementarity Program}
...
ready-to-run notebook are made available.\footnote{\url{https://github.com/AlioumAbdoulaye/md-cdbg}}
...
\begin{itemize}
\item \textbf{No-Deception:} the defender performs passive monitoring only and
...
\item \textbf{Random:} decoys are deployed uniformly at random across actions,
with no strategic reasoning. Comparing against it isolates the contribution of
\emph{optimized} placement over mere presence of decoys.
\item \textbf{Asghar-2025~\cite{asghar2025}:} the state-of-the-art GameSec~2025
...
We report three metrics. The first two are the \emph{discounted total
...
third metric is the \emph{violation rate}
\begin{equation}
...
methods we report the \emph{relative violation reduction}
...
\textbf{Resilience (Fig.~\ref{fig:vc}).} This is the headline result.
...
No-Deception, with the ordering
R-BNE-DO~$<$~Random~$<$~Asghar-2025~$<$~No-Deception. That Random outperforms
the more sophisticated Asghar-2025 on resilience is revealing: methods that
optimize a non-resilience objective can steer the system into low-resilience
states, whereas random placement at least spreads risk.

\textbf{Defender utility (Fig.~\ref{fig:utility}a).} R-BNE-DO+LCP dominates in
scenarios P ($5.56$ vs $5.25$) and E ($5.79$ vs $5.19$), and is statistically
indistinguishable from the best baseline in O. Even against the weakest attacker
(O), where raw utilities are comparable, our method still secures the system far
better (VC $0.18$ vs $0.60$): the defender obtains equal or higher utility
\emph{and} substantially better resilience, without trading one for the other.

\textbf{Attacker rationality (Fig.~\ref{fig:utility}b).} The zero-sum proxy of
Asghar-2025 drives the attacker utility to $\approx-1.8$ (an adversary forced
into self-harming moves), whereas our non-zero-sum treatment yields
$U^A\approx-0.6$, a rational attacker playing to its genuine incentives. This
gap signals that the zero-sum model misrepresents the threat, which can lead to
defenses tuned against an unrealistic adversary.

\textbf{Temporal behavior (Fig.~\ref{fig:resilience}).} R-BNE-DO+LCP stays above
$\Omega_{\min}$ most of the time and rebounds quickly after compromise
spikes---a direct effect of the fallback isolation policy---while the baselines
drift below the threshold and never recover.

\subsection{Sensitivity Analysis}

A natural concern is whether the resilience advantage is an artifact of the
particular parameter values chosen in Section~\ref{sec:setup}. To address this,
we vary the two parameters most directly tied to our contribution---the
resilience threshold $\Omega_{\min}$ and the global budget $B$---over wide
ranges, holding all else fixed, and re-measure the violation rate of every
method. Figure~\ref{fig:sensitivity} reports the results.

Across $\Omega_{\min}\in[0.45,0.65]$ (Fig.~\ref{fig:sensitivity}a),
R-BNE-DO+LCP remains the lowest-violation method at every operating point, and
the relative reduction with respect to Asghar-2025 stays in the
$70\%$--$77\%$ band throughout. As the threshold tightens, all methods
incur more violations, but the gap to the baselines does not close: the
constraint-aware fallback keeps R-BNE close to the threshold even when it is
demanding. Varying the budget $B\in[0.8,1.6]$ (Fig.~\ref{fig:sensitivity}b)
yields the same picture: the reduction with respect to Asghar-2025 stays in the
$71\%$--$77\%$ band, and the ordering
R-BNE-DO~$<$~Random~$<$~Asghar-2025~$<$~No-Deception never changes. Notably,
R-BNE already secures most of its benefit at a small budget, indicating that the
advantage stems from \emph{how} decoys are allocated under the equilibrium and
resilience constraint, not merely from \emph{how many} are deployed. These
sweeps confirm that the reported advantage is structural and robust to
reasonable reparameterization rather than tuned to a single configuration.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{fig_sensitivity.png}
\caption{Sensitivity of the violation rate to (a) the resilience threshold
$\Omega_{\min}$ and (b) the global budget $B$. R-BNE-DO+LCP remains the
lowest-violation method across the entire range, with a $70$--$77\%$ reduction
relative to Asghar-2025 throughout.}
\label{fig:sensitivity}
\end{figure*}

\subsection{Discussion}

Two higher-level lessons emerge. First, the comparison design
(Section~\ref{sec:results}) attributes the resilience gain to specific causes:
the gap to Asghar-2025 isolates the value of the non-zero-sum (LCP) treatment
and the resilience constraint, while the gap to Random isolates the value of
equilibrium-based placement over the mere presence of decoys; both gaps are
large, so neither component alone suffices. Second, the consistent ordering
across three very different attacker profiles indicates the advantage is
structural rather than tuned to a particular adversary.
...
in simulation, the framework is designed to carry over to deployed settings.
\emph{(i) Learning-based oracle:} the
...
of machine learning. \emph{(ii) Symmetric information:} a symmetric extension
...
\emph{(iii) Richer deception techniques:} the defender's action repertoire can be
...
new defender actions while preserving the
multi-domain coupling. \emph{(iv) Scalability:} approximate equilibrium solvers
