...
$a_{k,i}^D,a_{k,i}^A$ & $i$-th defender / attacker action; $\mathbf{s}$: state profile \\
...
$\delta_{\min}$ & Coherence threshold; \;$B$: budget \\
\bottomrule
\end{tabular}
\end{table}
...
utility \emph{and} substantially better resilience---it does not trade one for
the other.

\textbf{Attacker rationality (Fig.~\ref{fig:utility}b).} The zero-sum proxy of
Asghar-2025 drives the attacker utility to $\approx-1.8$, i.e. an adversary
...
\textbf{Temporal behavior (Fig.~\ref{fig:resilience}).} The resilience
